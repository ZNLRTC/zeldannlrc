...
foreach($empRecords as $row){
    ...
    $stmt = $conn->prepare("SELECT COUNT(*) AS hh_member FROM hh_members WHERE  ".$row['id']);
    $stmt->execute($searchArray);
    $records2 = $stmt->fetch();
    $totalRecordwithFilterhousehold = $records2['hh_member'];
   $data[] = array(
    "id"=>$count,
      "hh_code"=>$row['hh_code'],
      "sitio"=>$row['sitio'],
      "barangay"=>$row['barangay'],
      "hh_member"=>$totalRecordwithFilterhousehold,
      "action"=>"<button class='btn btn-sm btn-primary' id='manage_hh' hh-id =".$row['id']."><i class='fas fa-eye'i></i> View</button>
