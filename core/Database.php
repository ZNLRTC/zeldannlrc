<?php
	...
	class Database {
		...
		private function _buildLeftJoin() {
			...
			if ( $this->_leftJoin ) { // we have join to add

				foreach ( $this->_leftJoin as $leftjoin ) { // loop over joins array

					// add join to sql statement

					$this->_sql .= ' LEFT JOIN ' . $leftjoin['table'] . ' ON  ' .$this->_tableName.'.' . $leftjoin['tbl1'] . ' = '.$leftjoin['table'].'.'. $leftjoin['tbl2'] ;
		...
		private function _buildWhereLike() {
			...
			if ( $this->_whereLike ) { 
				...
				foreach ( $this->_whereLike as $whereLike ) { // loop over where array

					$columnVariableName = ':' . str_replace( '.', '', $whereLike['column'] );



					// add where column/value to sql statement

					$this->_sql .= ' ' . $whereLike['type'] . ' ' . $whereLike['column'] . '  LIKE ' . $columnVariableName;



					// map where vars to values to be used on execution

					$this->_executeParams[$columnVariableName] = $whereLike['value'];
		...
		public function leftjoin( $tableString, $tbl1, $tbl2 ) {
		...
		public function whereLike( $column, $value, $type = '' ) {
